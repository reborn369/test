...
### «Пушить заранее» и кнопка «Замерить»

Выстрел целится по настенным часам, а сеть открывается сама по себе. Между
«наши часы говорят сейчас» и «наши байты у секвенсора» стоят две вещи: насколько
врут часы машины и сколько летит пакет.

**Часы программа чинит сама.** Перед отсчётом она спрашивает время у публичных
серверов и дальше считает по исправленному времени — в логе появляется строка
`Clock: ...`. Ничего настраивать не надо. Если серверы недоступны, программа
скажет об этом и пойдёт по системным часам.

**Полёт покрывается упреждением.** Кнопка **Measure** рядом с полем меряет
задержку до RPC выбранной сети и сама подставляет число в **Push early, ms**.

Что делает это число: транзакции уходят на столько миллисекунд раньше срока и
повторяются, каждая с условием «не раньше времени старта». Ранние попытки
**отбивает сам узел** — газ не тратится, nonce не тратится, реверта нет.
Принятая попытка и есть минт. `0` — прежнее поведение, выстрел ровно в T0.
