...
// Four seconds are reserved for blocked UDP plus enough room for nonce/fee
// preparation. A clock check started only five seconds before T0 could consume
// the entire T-2 safety window and make the run late.
pub const CLOCK_SYNC_MIN_LEAD_MS: i64 = 12_000;
...
pub async fn measure_fire_lag(rpc: &RpcClient) -> Result<FireLagReport> {
    ...
    // The run itself applies the measured clock correction before counting
    // down. Adding the same offset to push lead would correct it twice and can
    // multiply conditional RPC traffic by an order of magnitude. Once the
    // clock is corrected, push lead only has to cover packet flight.
    if let Some(offset) = clock_offset_ms.filter(|value| value.abs() <= CLOCK_APPLY_CEILING_MS) {
        CLOCK_OFFSET_MS.store(offset, Ordering::Relaxed);
        CLOCK_CHECKED.store(true, Ordering::Relaxed);
    }
    let suggested_lead_ms = suggest_lead_ms(one_way_ms, Some(0));
    let summary = match clock_offset_ms {
        Some(off) if off.abs() >= 50 => format!(
            "clock {} {}ms (corrected) + flight {one_way_ms}ms → push early {suggested_lead_ms}ms",
